...
const PROCESSED_ATTR = "data-jxtento-launch-processed"
const RADAR_ROOT_ATTR = "data-jxtento-launch-dock"
...
function scanTimelineNames() {
  ...
  const usernameElements = document.querySelectorAll('[data-testid="User-Name"]:not([data-jxtento-tracked])');
  for (const el of Array.from(usernameElements)) {
    el.setAttribute("data-jxtento-tracked", "true");
...
async function scanSmartFollowers() {
  ...
  try {
    ...
    document.querySelector('.jxtento-intel-overlay')?.remove();

    // Inject JXtento Intel UI (similar to Frontrun style, natively injected)
    await injectJXtentoIntelDOM(header, handle);
...
async function injectAccountHistoryDOM(header: Element, handle: string) {
  ...
  try {
    ...
    console.error("[JXtento] Account history error:", err);
    container.remove();
  }
}
...
function injectLaunchRadarStyles() {
  ...
  if (document.querySelector("[data-jxtento-launch-style]")) return

  const style = document.createElement("style")
  style.setAttribute("data-jxtento-launch-style", "true")
...
function mountFloatingDock() {
  ...
  mount.innerHTML = `
    <div style="background:#fff;border:1px solid #111;color:#111;font-family:Arial,sans-serif;padding:12px;border-radius:2px">
      <div style="font-size:12px;font-weight:700;text-transform:uppercase;color:#6b6b66">JXtento</div>
...
async function fetchWatchlist() {
  try {
    ...
    console.warn("[JXtento] Failed to fetch watchlist:", err);
  }
}
...
function startWhenReady() {
  ...
  const start = async () => {
    ...
    try {
      ...
      console.warn("[JXtento] Error during X initialization:", err)
    }
  }
...
function XLaunchPanel({ context }: { context: XReplyContext }) {
  ...
  return (
    <aside className="mx-4 mb-3 mt-2 rounded-sm border border-jxtento-border bg-jxtento-panel p-3 text-jxtento-text shadow-none">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          type="button"
          className="rounded-sm border border-jxtento-border px-2 py-1 text-xs font-bold text-jxtento-text"
      ...
      {expanded ? (
        <div className="mt-3 space-y-3">
          ...
          <p className="rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs leading-5 text-jxtento-muted">
            {context.text}
          </p>
          ...
          <ul className="space-y-1 border-t border-jxtento-border pt-2">
            {draft.warnings.map((warning) => (
              <li key={warning} className="text-[11px] leading-4 text-jxtento-muted">
                {warning}
              </li>
            ))}
          </ul>
...
function XLaunchDock() {
  ...
  if (!context || !draft) {
    return (
      <div className="fixed bottom-4 right-4 z-[2147483647] w-80 rounded-sm border border-jxtento-border bg-jxtento-panel p-3 text-jxtento-text shadow-none">
        <p className="text-xs font-bold uppercase text-jxtento-muted">JXtento</p>
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[2147483647] w-[360px] max-w-[calc(100vw-2rem)] max-h-[calc(100vh-2rem)] overflow-y-auto rounded-sm border border-jxtento-border bg-jxtento-panel p-3 text-jxtento-text shadow-none flex flex-col">
      ...
      <p className="mt-3 max-h-20 overflow-hidden rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs leading-5 text-jxtento-muted">
        {context.text}
      </p>
      ...
      <button
        type="button"
        className="mt-3 w-full shrink-0 rounded-sm border border-jxtento-border px-2 py-2 text-xs font-bold text-jxtento-text hover:bg-jxtento-bg transition"
        onClick={() => setMinimized(true)}>
        Hide / Minimize
      </button>
    </div>
...
[data-jxtento-launch-dock] .fixed { position: fixed; }
[data-jxtento-launch-dock] .bottom-4 { bottom: 16px; }
[data-jxtento-launch-dock] .right-4 { right: 16px; }
[data-jxtento-launch-dock] .z-\\[2147483647\\] { z-index: 2147483647; }
[data-jxtento-launch-dock] .w-80 { width: 320px; }
[data-jxtento-launch-dock] .w-\\[360px\\] { width: 360px; }
[data-jxtento-launch-dock] .max-w-\\[calc\\(100vw-2rem\\)\\] { max-width: calc(100vw - 32px); }
[data-jxtento-launch-dock] .rounded-sm,
.axiom-x-launch-mount .rounded-sm { border-radius: 2px; }
[data-jxtento-launch-dock] .border,
.axiom-x-launch-mount .border { border-width: 1px; border-style: solid; }
[data-jxtento-launch-dock] .border-jxtento-border,
.axiom-x-launch-mount .border-jxtento-border { border-color: #27272a; }
[data-jxtento-launch-dock] .bg-jxtento-panel,
.axiom-x-launch-mount .bg-jxtento-panel { background: #121214; }
...
.axiom-x-launch-mount .text-jxtento-muted { color: #a1a1aa; }
[data-jxtento-launch-dock] .text-jxtento-accent,
.axiom-x-launch-mount .text-jxtento-accent { color: #3b82f6; }
[data-jxtento-launch-dock] .text-jxtento-good,
.axiom-x-launch-mount .text-jxtento-good { color: #008f5a; }
[data-jxtento-launch-dock] .p-2,
.axiom-x-launch-mount .p-2 { padding: 8px; }
[data-jxtento-launch-dock] .p-3,
.axiom-x-launch-mount .p-3 { padding: 12px; }
[data-jxtento-launch-dock] .px-2,
.axiom-x-launch-mount .px-2 { padding-left: 8px; padding-right: 8px; }
[data-jxtento-launch-dock] .px-3,
.axiom-x-launch-mount .px-3 { padding-left: 12px; padding-right: 12px; }
[data-jxtento-launch-dock] .py-1,
.axiom-x-launch-mount .py-1 { padding-top: 4px; padding-bottom: 4px; }
[data-jxtento-launch-dock] .py-2,
.axiom-x-launch-mount .py-2 { padding-top: 8px; padding-bottom: 8px; }
[data-jxtento-launch-dock] .m-0,
.axiom-x-launch-mount .m-0 { margin: 0; }
[data-jxtento-launch-dock] .mx-4,
.axiom-x-launch-mount .mx-4 { margin-left: 16px; margin-right: 16px; }
[data-jxtento-launch-dock] .mb-3,
.axiom-x-launch-mount .mb-3 { margin-bottom: 12px; }
[data-jxtento-launch-dock] .mt-1,
.axiom-x-launch-mount .mt-1 { margin-top: 4px; }
[data-jxtento-launch-dock] .mt-2,
.axiom-x-launch-mount .mt-2 { margin-top: 8px; }
[data-jxtento-launch-dock] .mt-3,
.axiom-x-launch-mount .mt-3 { margin-top: 12px; }
[data-jxtento-launch-dock] .flex,
.axiom-x-launch-mount .flex { display: flex; }
[data-jxtento-launch-dock] .grid,
.axiom-x-launch-mount .grid { display: grid; }
[data-jxtento-launch-dock] .grid-cols-2,
.axiom-x-launch-mount .grid-cols-2 { grid-template-columns: repeat(2, minmax(0, 1fr)); }
[data-jxtento-launch-dock] .items-start,
.axiom-x-launch-mount .items-start { align-items: flex-start; }
[data-jxtento-launch-dock] .items-center,
.axiom-x-launch-mount .items-center { align-items: center; }
[data-jxtento-launch-dock] .justify-between,
.axiom-x-launch-mount .justify-between { justify-content: space-between; }
[data-jxtento-launch-dock] .flex-wrap,
.axiom-x-launch-mount .flex-wrap { flex-wrap: wrap; }
[data-jxtento-launch-dock] .gap-2,
.axiom-x-launch-mount .gap-2 { gap: 8px; }
[data-jxtento-launch-dock] .gap-3,
.axiom-x-launch-mount .gap-3 { gap: 12px; }
[data-jxtento-launch-dock] .space-y-1 > * + *,
.axiom-x-launch-mount .space-y-1 > * + * { margin-top: 4px; }
[data-jxtento-launch-dock] .space-y-3 > * + *,
.axiom-x-launch-mount .space-y-3 > * + * { margin-top: 12px; }
[data-jxtento-launch-dock] .border-t,
.axiom-x-launch-mount .border-t { border-top-width: 1px; border-top-style: solid; }
[data-jxtento-launch-dock] .pt-2,
.axiom-x-launch-mount .pt-2 { padding-top: 8px; }
[data-jxtento-launch-dock] .text-\\[11px\\],
.axiom-x-launch-mount .text-\\[11px\\] { font-size: 11px; }
[data-jxtento-launch-dock] .text-xs,
.axiom-x-launch-mount .text-xs { font-size: 12px; }
[data-jxtento-launch-dock] .text-sm,
.axiom-x-launch-mount .text-sm { font-size: 14px; }
[data-jxtento-launch-dock] .text-base,
.axiom-x-launch-mount .text-base { font-size: 16px; }
[data-jxtento-launch-dock] .font-semibold,
.axiom-x-launch-mount .font-semibold { font-weight: 600; }
[data-jxtento-launch-dock] .font-bold,
.axiom-x-launch-mount .font-bold { font-weight: 700; }
[data-jxtento-launch-dock] .uppercase,
.axiom-x-launch-mount .uppercase { text-transform: uppercase; }
[data-jxtento-launch-dock] .leading-4,
.axiom-x-launch-mount .leading-4 { line-height: 16px; }
[data-jxtento-launch-dock] .leading-5,
.axiom-x-launch-mount .leading-5 { line-height: 20px; }
[data-jxtento-launch-dock] .leading-tight,
.axiom-x-launch-mount .leading-tight { line-height: 1.25; }
[data-jxtento-launch-dock] .truncate,
.axiom-x-launch-mount .truncate { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
[data-jxtento-launch-dock] .overflow-hidden,
.axiom-x-launch-mount .overflow-hidden { overflow: hidden; }
[data-jxtento-launch-dock] .max-h-20 { max-height: 80px; }
[data-jxtento-launch-dock] .shadow-none,
.axiom-x-launch-mount .shadow-none { box-shadow: none; }
[data-jxtento-launch-dock] button,
.axiom-x-launch-mount button {
  border: 1px solid #111111;
  cursor: pointer;
  font: inherit;
}
[data-jxtento-launch-dock] button:hover,
.axiom-x-launch-mount button:hover {
  background: #6b6b66;
}
[data-jxtento-launch-dock] .flex-col, .axiom-x-launch-mount .flex-col { flex-direction: column; }
[data-jxtento-launch-dock] .gap-1, .axiom-x-launch-mount .gap-1 { gap: 4px; }
[data-jxtento-launch-dock] .w-full, .axiom-x-launch-mount .w-full { width: 100%; }
[data-jxtento-launch-dock] .w-16, .axiom-x-launch-mount .w-16 { width: 64px; }
[data-jxtento-launch-dock] .h-16, .axiom-x-launch-mount .h-16 { height: 64px; }
[data-jxtento-launch-dock] .w-2, .axiom-x-launch-mount .w-2 { width: 8px; }
[data-jxtento-launch-dock] .h-2, .axiom-x-launch-mount .h-2 { height: 8px; }
[data-jxtento-launch-dock] .flex-1, .axiom-x-launch-mount .flex-1 { flex: 1 1 0%; }
[data-jxtento-launch-dock] .min-h-\\[60px\\], .axiom-x-launch-mount .min-h-\\[60px\\] { min-height: 60px; }
[data-jxtento-launch-dock] .object-cover, .axiom-x-launch-mount .object-cover { object-fit: cover; }
[data-jxtento-launch-dock] .cursor-pointer, .axiom-x-launch-mount .cursor-pointer { cursor: pointer; }
[data-jxtento-launch-dock] .rounded, .axiom-x-launch-mount .rounded { border-radius: 4px; }
[data-jxtento-launch-dock] .rounded-full, .axiom-x-launch-mount .rounded-full { border-radius: 9999px; }
[data-jxtento-launch-dock] .bg-jxtento-bad\\/10, .axiom-x-launch-mount .bg-jxtento-bad\\/10 { background-color: rgba(225, 29, 72, 0.1); }
[data-jxtento-launch-dock] .bg-jxtento-good\\/10, .axiom-x-launch-mount .bg-jxtento-good\\/10 { background-color: rgba(0, 143, 90, 0.1); }
[data-jxtento-launch-dock] .border-jxtento-bad\\/20, .axiom-x-launch-mount .border-jxtento-bad\\/20 { border-color: rgba(225, 29, 72, 0.2); }
[data-jxtento-launch-dock] .border-jxtento-good\\/20, .axiom-x-launch-mount .border-jxtento-good\\/20 { border-color: rgba(0, 143, 90, 0.2); }
[data-jxtento-launch-dock] .text-jxtento-bad, .axiom-x-launch-mount .text-jxtento-bad { color: #e11d48; }
[data-jxtento-launch-dock] .text-jxtento-warn, .axiom-x-launch-mount .text-jxtento-warn { color: #d97706; }
[data-jxtento-launch-dock] .underline, .axiom-x-launch-mount .underline { text-decoration-line: underline; }
[data-jxtento-launch-dock] .transition-colors, .axiom-x-launch-mount .transition-colors { transition-property: color, background-color, border-color, text-decoration-color, fill, stroke; transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1); transition-duration: 150ms; }
[data-jxtento-launch-dock] .transition-opacity, .axiom-x-launch-mount .transition-opacity { transition-property: opacity; transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1); transition-duration: 150ms; }
[data-jxtento-launch-dock] .hover\\:opacity-90:hover, .axiom-x-launch-mount .hover\\:opacity-90:hover { opacity: 0.9; }
[data-jxtento-launch-dock] .hover\\:bg-jxtento-muted:hover, .axiom-x-launch-mount .hover\\:bg-jxtento-muted:hover { background-color: #6b6b66; }
[data-jxtento-launch-dock] .focus\\:outline-none:focus, .axiom-x-launch-mount .focus\\:outline-none:focus { outline: 2px solid transparent; outline-offset: 2px; }
[data-jxtento-launch-dock] .py-1\\.5, .axiom-x-launch-mount .py-1\\.5 { padding-top: 6px; padding-bottom: 6px; }
[data-jxtento-launch-dock] .font-medium, .axiom-x-launch-mount .font-medium { font-weight: 500; }
[data-jxtento-launch-dock] .animate-pulse, .axiom-x-launch-mount .animate-pulse { animation: pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite; }
@keyframes pulse { 0%, 100% { opacity: 1; } 50% { opacity: .5; } }
@keyframes axiom-spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
[data-jxtento-launch-dock] .overflow-y-auto, .axiom-x-launch-mount .overflow-y-auto { overflow-y: auto; }
[data-jxtento-launch-dock] .max-h-\\[300px\\], .axiom-x-launch-mount .max-h-\\[300px\\] { max-height: 300px; }
`
