...
import { GuardedBadge } from "../components/GuardedBadge"
...
export default function JXtentoCSUI() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (changes: { [key: string]: chrome.storage.StorageChange }, areaName: string) => {
      if (areaName === "local" && changes["jxtento.settings"]) {
  ...
  useEffect(() => {
    ...
    let lastContextKey = ""
    console.log("[JXtento] CSUI mounted, initial URL:", lastUrl)

    const checkUrl = async (force = false) => {
      const currentUrl = window.location.href
      
      // Extract CA from JXtento Trade URL patterns:
      // https://jxtento.trade/meme/CA
      // https://jxtento.trade/token/CA
      // https://jxtento.trade/p/CA
      // https://jxtento.trade/?token=CA
      ...
      // JXtento Trade shows the original pump.fun link in the token detail page.
      ...
      // JXtento Trade often displays the pump.fun CA as text on the page
      ...
      // Priority: pump.fun page link > page text > JXtento URL (JXtento URL may be internal ID, not CA)
      ...
      if (tokenAddress) {
        ...
        const ignoredDomains = ["jxtento.trade", "x.com", "twitter.com", "t.me", "telegram.me", "telegram.org", "github.com", "solscan.io", "birdeye.so", "dexscreener.com", "pump.fun", "solana.com"]
        ...
        if (currentUrl !== lastUrl || contextKey !== lastContextKey) {
            console.log(`[JXtento] Saving new address/context to storage: ${tokenAddress} (source: ${tokenAddressFromPumpFun ? 'pump.fun link' : tokenAddressFromPageText ? 'page text' : 'axiom URL'})`)
            lastUrl = currentUrl
