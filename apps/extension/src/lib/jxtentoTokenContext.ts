...
export type JXtentoTokenContext = {
  address: string
  ticker?: string
  name?: string
  shortAddress?: string
  ...
  marketCapUsd?: number
  narrative?: string
  source: "jxtento-card" | "jxtento-link" | "manual"
}

export type JXtentoTokenCardMatch = {
  element: HTMLElement
  context: JXtentoTokenContext
}

const CARD_PROCESSED_ATTR = "data-jxtento-intel-card"
const MAX_CONTEXT_TEXT = 600

export function findJXtentoTokenCards(root: ParentNode = document): JXtentoTokenCardMatch[] {
  const candidates = new Map<string, JXtentoTokenCardMatch>()
  ...
  for (const element of elements) {
    ...
    const context = extractJXtentoTokenContext(card, address)
    if (!context) continue
...
export function extractJXtentoTokenContext(
  element: HTMLElement,
  address: string
): JXtentoTokenContext | null {
  ...
  const context: JXtentoTokenContext = {
    address,
...
function buildNarrative({
  ...
  const parts = [
    ticker ? `${ticker} token card on JXtento` : "Token card on JXtento",
