...
interface WalletTooltipProps {
  ...
  tokenContext?: JXtentoTokenContext | undefined
...
export function WalletTooltip({
  ...
  return (
    <div className="w-80 rounded-sm border border-jxtento-border bg-jxtento-panel p-3 text-jxtento-text shadow-none">
      <div className="mb-3 grid grid-cols-[1fr_auto] items-start gap-3 border-b border-jxtento-border pb-3">
        <div>
          <p className="text-xs uppercase text-jxtento-muted">
            {intelligence.type}
          </p>
        ...
        <span className="rounded-sm border border-jxtento-border px-2 py-1 text-xs font-bold text-jxtento-accent">
          {intelligence.badge}
        </span>
      </div>
      ...
      {intelligence.type === "wallet" ? (
        ...
        <div className="space-y-3">
          {tokenContext ? <JXtentoContextSummary context={tokenContext} /> : null}
...
function JXtentoContextSummary({ context }: { context: JXtentoTokenContext }) {
  return (
    <div className="rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs">
      <div className="flex items-center justify-between gap-2">
        <span className="font-bold uppercase text-jxtento-muted">JXtento card</span>
...
function ReputationSummary({
  ...
  if (loading) {
    return (
      <p className="rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs font-semibold text-jxtento-muted">
  ...
  if (!reputation) {
    return (
      <p className="rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs text-jxtento-muted">
  ...
  return (
    <div className="rounded-sm border border-jxtento-border bg-jxtento-bg p-2 text-xs">
      <div className="flex items-center justify-between gap-2">
        <span className="font-bold uppercase text-jxtento-muted">Proof audit</span>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 text-xs">
      <span className="text-jxtento-muted">{label}</span>
      <span className="font-semibold text-jxtento-text">{value}</span>
    </div>
  )
}
