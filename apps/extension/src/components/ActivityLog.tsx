  ...
  if (entry.type === "token_deployed") {
    return (
      <span className="text-xs font-semibold text-jxtento-text">
        ...
        {entry.name && (
          <span className="text-jxtento-muted font-normal ml-1">{entry.name}</span>
        )}
      </span>
    );
  }
  if (entry.type === "dev_buy") {
    return (
      <span className="text-xs font-semibold text-jxtento-text">
        Dev Buy{" "}
        {entry.devBuySol && (
          <span className="text-jxtento-muted font-normal">{entry.devBuySol} SOL</span>
        )}
      </span>
    );
  }
  if (entry.type === "wallet_connected") {
    return (
      <span className="text-xs font-semibold text-jxtento-text">
        Wallet Connected{" "}
        {entry.provider && (
          <span className="text-jxtento-muted font-normal capitalize">{entry.provider}</span>
        )}
      </span>
    );
  }
  if (entry.type === "wallet_disconnected") {
    return <span className="text-xs font-semibold text-jxtento-text">Wallet Disconnected</span>;
  }
  return null;
}
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      <div className="flex items-center justify-between pb-2 border-b border-jxtento-border/10">
        <h2 className="text-sm font-semibold text-jxtento-text">Activity Log</h2>
        {entries.length > 0 && (
          <button
            onClick={handleClear}
            className="text-[10px] text-jxtento-muted hover:text-jxtento-danger transition-colors"
          >
            Clear
          </button>
        )}
      ...
      <div className="flex flex-col gap-2 overflow-y-auto max-h-[500px] pb-10 mt-2">
        {loading ? (
          <div className="text-center py-10 text-jxtento-muted text-xs">Loading...</div>
        ) : entries.length === 0 ? (
          ...
          entries.map((entry) => (
            <div
              key={entry.id}
              className="p-3 rounded-lg border bg-jxtento-bg border-jxtento-border/20"
            >
              <div className="flex justify-between items-start mb-1">
                ...
                <span className="text-[10px] text-jxtento-muted whitespace-nowrap ml-2">
              ...
              {entry.publicKey && entry.type === "wallet_connected" && (
                <p className="text-[10px] text-jxtento-muted font-mono mt-1">
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  {entry.mintAddress && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-[10px] text-jxtento-muted font-mono hover:text-jxtento-text transition-colors"
                    >
      ...
      <p className="text-[9px] text-jxtento-muted/50 text-center mt-auto">
