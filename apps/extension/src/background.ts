...
async function ensureActiveTab(): Promise<number> {
  ...
  throw new Error("Please open a regular web page (e.g. pump.fun or jxtento.trade) first. Extensions cannot connect to wallets on browser settings pages.");
}
...
function connectGuardWs(wallet: string) {
  ...
  guardWs.onclose = () => {
    ...
    guardWs = null;
    // Attempt reconnect after 5s if still have consent
    setTimeout(() => {
      chrome.storage.local.get(["exitGuardConsent", "walletSession"], (res) => {
        if (res.exitGuardConsent && res.walletSession?.publicKey) {
          connectGuardWs(res.walletSession.publicKey);
        }
      });
    }, 5000);
  };
...
chrome.storage.local.onChanged.addListener((changes) => {
  if (changes.exitGuardConsent || changes.walletSession) {
    chrome.storage.local.get(["exitGuardConsent", "walletSession"], (res) => {
      if (res.exitGuardConsent && res.walletSession?.publicKey) {
        connectGuardWs(res.walletSession.publicKey);
      } else if (guardWs) {
